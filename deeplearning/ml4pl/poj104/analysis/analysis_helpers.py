...
def plot_logs(logs, hyps, lower_ylim=0.85, display_list=None, legend_loc='best'):
    ...
    for name, log in logs.items():
        ...
        plt.plot(log['epoch'].values, log['valid_acc'].values, label='_nolegend_', ls='dashed', c=c) # , label=make_label(name, 'valid')
        plt.plot(log['epoch'].values, log['train_acc'].values, label='_nolegend_', ls='dotted', c=c) # make_label(name, 'train')
        if hasattr(log, 'test_acc'):
            ...
            annot_max(log['epoch'].values, log['valid_acc'].values, log['test_acc'].values, c, label=label)
